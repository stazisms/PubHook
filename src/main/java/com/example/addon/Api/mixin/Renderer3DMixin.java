...
import com.example.addon.Api.Elite.IMesh;
import com.example.addon.Api.Elite.IRenderer3D;
...
@Mixin(value = {Renderer3D.class}, remap = false)
public abstract class Renderer3DMixin implements IRenderer3D {
    ...
    public void triangle(double x1, double y1, double z1, double x2, double y2, double z2, double x3, double y3, double z3, Color top, Color left, Color right) {
        ((IMesh) this.triangles).triangle(this.triangles.vec3(x1, y1, z1).color(top).next(), this.triangles.vec3(x2, y2, z2).color(left).next(), this.triangles.vec3(x3, y3, z3).color(right).next());
    }

...
public class BlockUtils2 {
    ...
    public static void justPlace(FindItemResult item, BlockHitResult hitResult, boolean swing, boolean rotate, int rotationPriority) {
        if (rotate) {
            ...
            boolean notSelected = !item.isOffhand() && MeteorClient.mc.player.getInventory().getSelectedSlot() != item.slot();
            ...
            if (notSelected) {
                MeteorClient.mc.player.networkHandler.sendPacket(new UpdateSelectedSlotC2SPacket(MeteorClient.mc.player.getInventory().getSelectedSlot()));
            }
        }
    }

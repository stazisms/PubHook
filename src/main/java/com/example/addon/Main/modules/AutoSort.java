...
public class AutoSort extends Hooked {
    ...
    public AutoSort() {
        super(Hook.CATEGORY, "auto-sort", "Sorts your inventory. May cause desync on some servers.");
    }
    ...
    private List<Setting<Item>> initSettings() {
        ...
        for (int i = 0; i < 36; ++i) {
            ...
            ItemSetting itemSetting = new Builder().name(baseName + i).description(baseDescription + i).defaultValue(Items.AIR).build();
            switch (cat) {
                case 0:
    ...
    private void moveItems() {
        Setting<Item> invSlot = this.settingList.get(this.begin);
        if (invSlot.get() != Items.AIR && this.mc.player.getInventory().getStack(this.begin).getItem() != invSlot.get()) {
            FindItemResult getSlot = this.searchInv(invSlot.get());
    ...
    private void instantMoveItems() {
        for (int i = 0; i < 36; ++i) {
            Setting<Item> invSlot = this.settingList.get(i);
            if (invSlot.get() != Items.AIR && this.mc.player.getInventory().getStack(i).getItem() != invSlot.get()) {
                int slot = this.searchInv(invSlot.get()).slot();

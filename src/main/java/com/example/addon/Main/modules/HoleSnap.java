...
import com.example.addon.Api.util.holes.Hole;
import com.example.addon.Api.util.holes.HoleUtils;
import com.example.addon.Api.util.holes.enums.HoleType;
...
import static com.example.addon.Api.util.BlockHelper.inside;
import static com.example.addon.Api.util.GANGTIL.collidable;

public class HoleSnap extends Hooked {
    public HoleSnap() {
        super(Hook.CATEGORY, "Hole Snap", "For the times when you cant even press W.");
    }
        ...
        if (mc.player != null && mc.world != null) {
            ...
            if (hole != null && !singleBlocked()) {
                ...
                if (mc.player.getX() == hole.middle.x && mc.player.getZ() == hole.middle.z) {
                    ...
                    } else if (inside(mc.player, mc.player.getBoundingBox().offset(0, -0.05, 0))) {
                        this.toggle();
                    ...
                    if (inside(mc.player, mc.player.getBoundingBox().offset(x, 0, z))) {
                        collisions++;
                    ...
                    if (ticks > 0) {
                        ticks--;
                    } else if (inside(mc.player, mc.player.getBoundingBox().offset(0, -0.05, 0)) && jump.get()) {
